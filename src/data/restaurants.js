    ...
    menu: [
      {
        ...
        price: '$9.25',
        matchTags: ['Vegan', 'Vegetarian', 'High Protein', 'Savory', 'Dairy-Free'],
        avoidTags: [],
        emoji: '🥣',
      },
      {
...
// Score a restaurant against a group of people + vibe
export function scoreRestaurant(restaurant, people, vibe = []) {
  ...
  for (const person of people) {
    ...
    const matchingItems = restaurant.menu.filter(item => {
      const hasAvoid = avoidTags.some(a => item.avoidTags.includes(a) || item.matchTags.includes(a))
      if (hasAvoid) return false
      const flavorMatch = allTags.filter(t => item.matchTags.includes(t)).length
      return flavorMatch > 0
    })
    personScore += matchingItems.length * 10

    if (person.budget) {
      const budgetNum = parseInt(person.budget.replace('$', '').replace('+', ''))
      const menuPrices = restaurant.menu.map(m => parseFloat(m.price.replace('$', '')))
      const cheapestItem = Math.min(...menuPrices)
      if (cheapestItem <= budgetNum) personScore += 20
    }
