      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div className="flex gap-3">
              {socialLinks.map((s) => (
                <a
                  ...
                  href={s.href}
                  aria-label={s.label}
                  className="w-9 h-9 rounded-xl bg-white/10 hover:bg-white/20 text-gray-400 hover:text-white flex items-center justify-center transition-all duration-200"
                >
                  {s.icon}
                </a>
              ))}
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-gray-500 text-sm">
            © {new Date().getFullYear()} Social Plate-Share. All rights reserved.
          </p>
          <div className="flex gap-6">
            {legalLinks.map((l) => (
              <a key={l.label} href={l.href} className="text-gray-500 hover:text-gray-300 text-xs transition-colors">
                {l.label}
              </a>
            ))}
          </div>
